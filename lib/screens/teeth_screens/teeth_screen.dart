...
class TeethScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        ...
        body: BlocConsumer<TeethScreenBloc, TeethScreenState>(
          listener: (context, state) {
            if (state is TeethScreenLoadingState) {
              showDialog(
                context: context,
            ...
            if (state is TeethScreenColorsState) {
              teethColors = state.teethColors;
...
Future<List<Color>> colorStatus() async {
  final userId = Supabase.instance.client.auth.currentUser!.id;
  List<Tooth> toothList = await getToothStatus(userId);
  List<Color> teethColors = [
    Colors.white,
    Colors.white,
    Colors.white,
  ...
  for (var i = 0; i < toothList.length; i++) {
    var toothNum = int.tryParse(toothList[i].toothNo!);
    if (toothList[i].toothStatus == statusList[0]) {
      teethColors[toothNum! - 1] = const Color(0xffCE2164);
    }
    if (toothList[i].toothStatus == statusList[1]) {
      teethColors[toothNum! - 1] = const Color(0xff9F571C);
    }
    if (toothList[i].toothStatus == statusList[2]) {
      teethColors[toothNum! - 1] = const Color(0xffB322C0);
    }
    if (toothList[i].toothStatus == statusList[3]) {
      teethColors[toothNum! - 1] = const Color(0xff393C3E);
    }
    if (toothList[i].toothStatus == statusList[4]) {
      teethColors[toothNum! - 1] = const Color(0xff7644CC);
    }
    if (toothList[i].toothStatus == statusList[5]) {
      teethColors[toothNum! - 1] = const Color(0xffE91212);
    }
    if (toothList[i].toothStatus == statusList[6]) {
      teethColors[toothNum! - 1] = const Color(0xff9C6871);
    }
    if (toothList[i].toothStatus == statusList[7]) {
      teethColors[toothNum! - 1] = const Color(0xff0D7E77);
    }
  }
  return teethColors;
}
...
bottomSheetWidget(
  ...
  final userId = Supabase.instance.client.auth.currentUser!.id;
  final tooth = await getToothStatusById(userId, toothNo);
  if (tooth != null) {
